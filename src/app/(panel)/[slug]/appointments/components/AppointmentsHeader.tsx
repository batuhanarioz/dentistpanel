            ...
            <div className="flex flex-col xs:flex-row xs:items-center gap-3">
                <div className="flex items-center gap-2">
                    <div className="flex items-center rounded-lg border border-slate-200 bg-white overflow-hidden shadow-sm shrink-0">
                        <button
                            onClick={onPrev}
                            className="flex h-9 w-9 items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition-all border-r border-slate-100"
                        ...
                        <button
                            onClick={onNext}
                            className="flex h-9 w-9 items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition-all"
                    ...
                    {/* Date label with loading spinner */}
                ...
                <label className="relative flex items-center gap-2 h-9 px-3 rounded-lg border border-slate-200 bg-white hover:border-indigo-200 hover:bg-indigo-50/10 transition-all cursor-pointer shadow-sm group min-w-fit">
                    ...
                    <span className="text-sm font-bold text-slate-700 whitespace-nowrap group-hover:text-indigo-600">
                        {view === "week" ? weekRangeStr : format(selectedDate, "d MMMM yyyy, EEEE", { locale: tr })}
                    </span>
                    <div className="hidden lg:block">
                        {loading && (
                            <span className="ml-1 h-3 w-3 rounded-full border-2 border-indigo-400 border-t-transparent animate-spin" />
                        )}
                    </div>
                    <input
                        type="date"
                        ...
                        onChange={(e) => {
                            ...
                            if (parts.length === 3) {
                                onDateChange(new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2])));
                            }
                        }}
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                ...
                <div className="relative">
                    <button
                        ...
                        className="flex items-center gap-2 h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 hover:border-indigo-200 transition-all outline-none focus:ring-2 focus:ring-indigo-500/10 shadow-sm"
                    >
                        ...
                        <div className="flex items-center justify-center bg-indigo-50 text-indigo-600 px-1.5 py-0.5 rounded-md text-[10px] font-black border border-indigo-100 min-w-[20px]">
                            {totalCount}
                        </div>
                            ...
                            <div className="absolute left-0 lg:left-auto lg:right-0 mt-2 w-60 bg-white border border-slate-200 rounded-xl shadow-2xl z-[70] py-2 overflow-hidden animate-in fade-in zoom-in duration-150 ring-1 ring-black/5">
                                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between mb-1 bg-slate-50/50">
                                    ...
                                    <div className="flex gap-3">
                                        <button onClick={selectAll} className="text-[10px] text-indigo-600 font-bold hover:text-indigo-700">Tümü</button>
                                ...
                                <div className="max-h-72 overflow-y-auto px-1">
                                    {doctors.map(d => (
                                        ...
                                        >
                                            <div className={`h-4.5 w-4.5 rounded-md border flex items-center justify-center transition-all ${selectedDoctorIds.includes(d.id) ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'border-slate-300 bg-white group-hover:border-indigo-300'}`}>
