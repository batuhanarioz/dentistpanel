        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between bg-white border-b border-slate-200 px-4 py-4 lg:py-3 shadow-sm flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
                <div className="flex items-center rounded-lg border border-slate-200 bg-white overflow-hidden shadow-sm shrink-0">
                    <button
                        onClick={onPrev}
                        className="flex h-9 w-9 items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-teal-600 transition-all border-r border-slate-100"
                    ...
                    <button
                        onClick={onToday}
                        className={`h-9 px-4 text-[11px] font-bold transition-all border-r border-slate-100 ${isToday
                    ...
                    <button
                        onClick={onNext}
                        className="flex h-9 w-9 items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-teal-600 transition-all"
                ...
                <div className="relative flex-1 min-w-0">
                    <button
                        ...
                        className={`flex items-center gap-1.5 h-9 px-2.5 w-full rounded-lg border shadow-sm transition-all group ${datePickerOpen ? "border-teal-400 bg-teal-50/30" : "border-slate-200 bg-white hover:border-teal-300 hover:bg-teal-50/20"}`}
                    >
                        <svg className="h-3.5 w-3.5 text-teal-500 shrink-0 group-hover:text-teal-600 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                        ...
                        <span className="sm:hidden text-xs font-bold text-slate-700 truncate group-hover:text-teal-600">
                            {view === "week"
                                ? weekRangeStr.replace(/\s\d{4}$/, "")
                                : format(selectedDate, "d MMM, EEE", { locale: tr })}
                        </span>
                        {/* Desktop: full format */}
                        <span className="hidden sm:inline text-sm font-bold text-slate-700 truncate group-hover:text-teal-600">
                            {view === "week" ? weekRangeStr : format(selectedDate, "d MMMM yyyy, EEEE", { locale: tr })}
                        </span>
                        {loading && (
                            <span className="ml-auto shrink-0 h-3 w-3 rounded-full border-2 border-teal-400 border-t-transparent animate-spin" />
                        )}
                    </button>
                    <PremiumDatePicker
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                {onNewAppointment && (
                    <button
                        ...
                        className="flex items-center gap-2 h-9 px-4 rounded-lg bg-teal-600 hover:bg-teal-700 text-white text-xs font-black shadow-sm shadow-teal-200 transition-all active:scale-95"
                    >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor">
                ...
                <div className="relative">
                    <button
                        ...
                        className="flex items-center gap-2 h-9 px-3 rounded-lg border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 hover:border-teal-200 transition-all outline-none focus:ring-2 focus:ring-indigo-500/10 shadow-sm"
                    >
                        ...
                        <div className="flex items-center justify-center bg-teal-50 text-teal-600 px-1.5 py-0.5 rounded-md text-[10px] font-black border border-teal-100 min-w-[20px]">
                            {totalCount}
                        </div>
                            ...
                            <div className="absolute left-0 lg:left-auto lg:right-0 mt-2 w-60 bg-white border border-slate-200 rounded-xl shadow-2xl z-[70] py-2 overflow-hidden animate-in fade-in zoom-in duration-150 ring-1 ring-black/5">
                                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between mb-1 bg-slate-50/50">
                                    ...
                                    <div className="flex gap-3">
                                        <button onClick={selectAll} className="text-[10px] text-teal-600 font-bold hover:text-indigo-700">Tümü</button>
                                        <button onClick={deselectAll} className="text-[10px] text-slate-500 font-bold hover:text-slate-700">Hiçbiri</button>
                                    </div>
                                </div>
                                <div className="max-h-72 overflow-y-auto px-1">
                                    {doctors.map(d => (
                                        ...
                                        >
                                            <div className={`h-4.5 w-4.5 rounded-md border flex items-center justify-center transition-all ${selectedDoctorIds.includes(d.id) ? 'bg-teal-600 border-teal-600 text-white shadow-sm' : 'border-slate-300 bg-white group-hover:border-teal-300'}`}>
                                            ...
                                            <span className="text-[10px] font-black text-slate-400 bg-slate-50 border border-slate-100 px-1.5 py-0.5 rounded-md tabular-nums min-w-[24px] text-center">
                                                ({eventCounts[d.id] || 0})
                                            </span>
                                        </button>
                                    ))}
