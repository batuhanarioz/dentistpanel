...
import { useGuide, useUpdateProtocol } from "@/hooks/useGuide";
        ...
        <ul className="space-y-2">
            {lines.map((line, i) => {
                ...
                return (
                    ...
                    >
                        <div
                            className={`mt-0.5 shrink-0 w-4 h-4 rounded shadow-sm border flex items-center justify-center transition-colors ${isChecked ? "border-transparent" : "bg-white border-slate-300"}`}
                            style={isChecked ? { background: brandFrom } : {}}
                        >
                            {isChecked && (
            ...
            >
                ...
                <div className="flex items-start justify-between p-5 border-b border-slate-100">
                    <div>
                        ...
                        {item.custom_notes && (
                            <span className="inline-flex items-center gap-1 text-xs font-medium mt-1" style={{ color: brandFrom }}>
                    ...
                    {!editing ? (
                        ...
                        <>
                            ...
                            <textarea
                                ...
                                rows={14}
                                className="w-full border border-slate-200 rounded-xl p-3 text-sm text-slate-700 resize-none focus:outline-none transition-all"
                                style={{ borderColor: draft ? brandFrom : undefined }}
                            ...
                            <div className="flex gap-2 mt-3">
                                <button
                                    onClick={handleSave}
                                    disabled={isPending}
                                    className="flex-1 text-white text-sm font-medium py-2 rounded-xl transition-colors disabled:opacity-60"
...
export default function GuidePage() {
    const { data: items = [], isLoading, error } = useGuide();
    const clinic = useClinic();
    const { userRole, isSuperAdmin, is_clinical_provider } = clinic;
    ...
    if (error) {
        return (
            <div className="flex items-center justify-center h-64 text-slate-400">
                <p>Kılavuz yüklenemedi. Lütfen sayfayı yenileyin.</p>
            </div>
        );
    }
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-5">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        className="w-full pl-9 pr-4 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none bg-white transition-all"
                        style={{ borderColor: search ? brandFrom : undefined }}
                    />
                </div>
                    ...
                    {categories.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                style={isActive && !col ? { background: brandFrom } : {}}
                            >
                                {cat}
                            </button>
                        );
...
function TreatmentCard({ item, onClick }: { item: TreatmentLibraryItem; onClick: () => void }) {
    ...
    return (
        <button
            onClick={onClick}
            className="group text-left bg-white border border-slate-100 rounded-2xl p-4 transition-all relative"
            style={{ '--hover-border': brandFrom } as any}
        >
            <style jsx>{`
                button:hover { border-color: var(--hover-border); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); }
            `}</style>
            {hasCustom && (
                <span className="absolute top-2.5 right-2.5 w-2 h-2 rounded-full" style={{ background: brandFrom }} title="Klinik özel protokol" />
            )}
            <span className={`inline-block text-[11px] font-medium px-2 py-0.5 rounded-full mb-2 ${col.bg} ${col.text}`}>
                {item.category}
            </span>
            <p className="text-sm font-semibold text-slate-700 leading-snug transition-colors group-hover:text-slate-900">
                {item.name}
            </p>
            {item.protocol_notes && (
