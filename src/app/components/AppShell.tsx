...
function NavTooltip({ label, show, children }: { label: string; show: boolean; children: React.ReactNode }) {
  ...
  return (
    <div className="relative group/ntt">
      {children}
      <div className="pointer-events-none absolute left-full top-1/2 -translate-y-1/2 ml-3 z-[300] opacity-0 scale-95 group-hover/ntt:opacity-100 group-hover/ntt:scale-100 transition-all duration-150 whitespace-nowrap">
        <div className="bg-slate-900 text-white text-[11px] font-bold px-3 py-1.5 rounded-xl shadow-xl">
          {label}
        </div>
        <div className="absolute -left-1 top-1/2 -translate-y-1/2 h-2 w-2 rotate-45 bg-slate-900" />
      </div>
    </div>
  );
}
...
function SuperAdminNav({
  ...
  const activeStyle = { backgroundColor: '#0f172a', color: '#fff', boxShadow: '0 10px 20px -5px rgba(15, 23, 42, 0.3)' };

  const items = [
    { href: "/platform/clinics/monitoring", label: "USS Monitoring", d: "M5.25 14.25h13.5m-13.5 0a3 3 0 0 1-3-3m3 3a3 3 0 1 0 6 0m-6 0H3m16.5 0a3 3 0 0 0 3-3m-3 3a3 3 0 1 1-6 0m6 0h1.5m-1.5 0H12m-8.457 3.077 1.41-.513m14.095-5.13 1.41-.513M5.106 17.785l1.15-.964m11.49-9.642 1.149-.964M7.501 19.795l.75-1.3M18.497 19.795l-.75-1.3m-11.5-14.99.75 1.3m11.499-1.3-.75 1.3M7.5 4.205l-.75 1.3m11.499-1.3-.75 1.3M2.25 8.25l1.41.513m16.879 6.514 1.41.513M3.659 5.173l1.15.964m13.532 11.57 1.15.964" },
    { href: "/platform/clinics/activity", label: "Aktivite & Bildirimler", d: "M14.857 17.082a23.848 23.848 0 0 0 5.454-1.31A8.967 8.967 0 0 1 18 9.75V9A6 6 0 0 0 6 9v.75a8.967 8.967 0 0 1-2.312 6.022c1.733.64 3.56 1.085 5.455 1.31m5.714 0a24.255 24.255 0 0 1-5.714 0m5.714 0a3 3 0 1 1-5.714 0" },
    { href: "/platform/clinics/manage", label: "Klinik Yönetimi", d: "M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125-1.125V21" },
    { href: "/platform/users", label: "Kullanıcı Yönetimi", d: "M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" },
    { href: "/platform/clinics/addons", label: "Eklentiler", d: "M21 7.5l-2.25-1.313M21 7.5v2.25m0-2.25l-2.25 1.313M3 7.5l2.25-1.313M3 7.5l2.25 1.313M3 7.5v2.25m9 3l2.25-1.313M12 12.75l-2.25-1.313M12 12.75V15m0 6.75l2.25-1.313M12 21.75V19.5m0 2.25l-2.25-1.313m0-16.875L12 2.25l2.25 1.313M21 14.25v2.25l-2.25 1.313m-13.5 0L3 16.5v-2.25" },
    { href: "/platform/clinics", label: "Sistem Ayarları", d: "M9.594 3.94c.09-.542.56-.94 1.11-.94h2.593c.55 0 1.02.398 1.11.94l.213 1.281c.063.374.313.686.645.87.074.04.147.083.22.127.325.196.72.257 1.075.124l1.217-.456a1.125 1.125 0 0 1 1.37.49l1.296 2.247a1.125 1.125 0 0 1-.26 1.431l-1.003.827c-.293.241-.438.613-.43.992a7.723 7.723 0 0 1 0 .255c-.008.378.137.75.43.991l1.004.827c.424.35.534.955.26 1.43l-1.298 2.247a1.125 1.125 0 0 1-1.369.491l-1.217-.456c-.355-.133-.75-.072-1.076.124a6.47 6.47 0 0 1-.22.128c-.331.183-.581.495-.644.869l-.213 1.281c-.09.543-.56.94-1.11.94h-2.594c-.55 0-1.019-.398-1.11-.94l-.213-1.281c-.062-.374-.312-.686-.644-.87a6.52 6.52 0 0 1-.22-.127c-.325-.196-.72-.257-1.076-.124l-1.217.456a1.125 1.125 0 0 1-1.369-.49l-1.297-2.247a1.125 1.125 0 0 1 .26-1.431l1.004-.827c.292-.24.437-.613.43-.991a6.932 6.932 0 0 1 0-.255c.007-.38-.138-.751-.43-.992l-1.004-.827a1.125 1.125 0 0 1-.26-1.43l1.297-2.247a1.125 1.125 0 0 1 1.37-.491l1.216.456c.356.133.751.072 1.076-.124.072-.044.146-.086.22-.128.332-.183.582-.495.644-.869l.214-1.28ZM15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" },
  ];
    ...
    <>
      ...
      <div className="space-y-1">
        {items.map(({ href, label, d }) => (
          <NavTooltip key={href} label={label} show={!!collapsed}>
            <Link href={href} className={linkClass(href)} onClick={handleClick} style={isActive(href) ? activeStyle : {}}>
              {isActive(href) && <div className="absolute left-0 top-3 bottom-3 w-1.5 rounded-r-full bg-white/40" />}
              <svg className={`h-4 w-4 shrink-0 transition-colors ${isActive(href) ? "text-white" : ""}`} fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
...
function ClinicNav({
  ...
  const NavHeader = ({ children }: { children: React.ReactNode }) => {
    if (collapsed) return <div className="my-1 mx-auto w-6 border-t border-slate-200/80" />;
    return (
  ...
  const activeStyle = (href: string) => isActive(href) ? { backgroundColor: 'var(--brand-from)', color: '#fff', boxShadow: `0 10px 20px -5px ${clinic.themeColorFrom}40` } : {};
  const iconCls = (href: string) => `h-4 w-4 shrink-0 transition-colors ${isActive(href) ? "text-white" : ""}`;
  const indicator = (href: string) => isActive(href) && <div className="absolute left-0 top-3 bottom-3 w-1.5 rounded-r-full bg-white/40" />;
  const c = !!collapsed;
      ...
      <div className={collapsed ? "space-y-1" : "space-y-4"}>
        ...
        <div>
          ...
          <div className="space-y-1 mt-1">
            <NavTooltip label="Genel Bakış" show={c}>
              <Link id="tour-dashboard" href={base} className={linkClass(base)} style={activeStyle(base)} onClick={handleClick}>
                {indicator(base)}
                <svg className={iconCls(base)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m2.25 12 8.954-8.955c.44-.439 1.152-.439 1.591 0L21.75 12M4.5 9.75v10.125c0 .621.504 1.125 1.125 1.125H9.75v-4.875c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125V21h4.125c.621 0 1.125-.504 1.125-1.125V9.75M8.25 21h8.25" /></svg>
                {!c && <span>Genel Bakış</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Randevu Yönetimi" show={c}>
              <Link id="tour-appointment-management" href={`${base}/appointment-management`} className={linkClass(`${base}/appointment-management`)} style={activeStyle(`${base}/appointment-management`)} onClick={handleClick}>
                {indicator(`${base}/appointment-management`)}
                <svg className={iconCls(`${base}/appointment-management`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 0 1 2.25-2.25h13.5A2.25 2.25 0 0 1 21 7.5v11.25m-18 0A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75m-18 0v-7.5A2.25 2.25 0 0 1 5.25 9h13.5A2.25 2.25 0 0 1 21 11.25v7.5m-9-6h.008v.008H12v-.008ZM12 15h.008v.008H12V15Zm0 2.25h.008v.008H12v-.008ZM9.75 15h.008v.008H9.75V15Zm0 2.25h.008v.008H9.75v-.008ZM7.5 15h.008v.008H7.5V15Zm0 2.25h.008v.008H7.5v-.008Zm6.75-4.5h.008v.008h-.008v-.008Zm0 2.25h.008v.008h-.008V15Zm0 2.25h.008v.008h-.008v-.008Zm2.25-4.5h.008v.008H16.5v-.008Zm0 2.25h.008v.008H16.5V15Z" /></svg>
                {!c && <span>Randevu Yönetimi</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Ödeme Yönetimi" show={c}>
              <Link id="tour-payment-management" href={`${base}/payment-management`} className={linkClass(`${base}/payment-management`)} style={activeStyle(`${base}/payment-management`)} onClick={handleClick}>
                {indicator(`${base}/payment-management`)}
                <svg className={iconCls(`${base}/payment-management`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" /></svg>
                {!c && <span>Ödeme Yönetimi</span>}
              </Link>
            </NavTooltip>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1 mt-1">
            <NavTooltip label="Hasta Kayıtları" show={c}>
              <Link id="tour-patients" href={`${base}/patients`} className={linkClass(`${base}/patients`)} style={activeStyle(`${base}/patients`)} onClick={handleClick}>
                {indicator(`${base}/patients`)}
                <svg className={iconCls(`${base}/patients`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 0 0 2.625.372 9.337 9.337 0 0 0 4.121-.952 4.125 4.125 0 0 0-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 0 1 8.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0 1 11.964-3.07M12 6.375a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0Zm8.25 2.25a2.625 2.625 0 1 1-5.25 0 2.625 2.625 0 0 1 5.25 0Z" /></svg>
                {!c && <span>Hasta Kayıtları</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Laboratuvar" show={c}>
              <Link href={`${base}/lab-management`} className={linkClass(`${base}/lab-management`)} style={activeStyle(`${base}/lab-management`)} onClick={handleClick}>
                {indicator(`${base}/lab-management`)}
                <svg className={iconCls(`${base}/lab-management`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23-.693L5 14.5m14.8.8 1.402 1.402c1 1 .03 2.798-1.328 2.798H6.126c-1.358 0-2.329-1.798-1.328-2.798L5 14.5" /></svg>
                {!c && <span>Laboratuvar</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Tedavi Planları" show={c}>
              <Link href={`${base}/treatment-plans`} className={linkClass(`${base}/treatment-plans`)} style={activeStyle(`${base}/treatment-plans`)} onClick={handleClick}>
                {indicator(`${base}/treatment-plans`)}
                <svg className={iconCls(`${base}/treatment-plans`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" /></svg>
                {!c && <span>Tedavi Planları</span>}
              </Link>
            </NavTooltip>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-1 mt-1">
            <NavTooltip label="Online Randevu" show={c}>
              <Link href={`${base}/online-bookings`} className={linkClass(`${base}/online-bookings`)} style={activeStyle(`${base}/online-bookings`)} onClick={handleClick}>
                {indicator(`${base}/online-bookings`)}
                <svg className={iconCls(`${base}/online-bookings`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25m18 0A2.25 2.25 0 0018.75 3H5.25A2.25 2.25 0 003 5.25m18 0H3" /></svg>
                {!c && <span>Online Randevu</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Hasta İletişimi" show={c}>
              <Link href={`${base}/communication`} className={linkClass(`${base}/communication`)} style={activeStyle(`${base}/communication`)} onClick={handleClick}>
                {indicator(`${base}/communication`)}
                ...
                {!c && <span>Hasta İletişimi</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Klinik Kılavuzu" show={c}>
              <Link href={`${base}/guide`} className={linkClass(`${base}/guide`)} style={activeStyle(`${base}/guide`)} onClick={handleClick}>
                {indicator(`${base}/guide`)}
                <svg className={iconCls(`${base}/guide`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" /></svg>
                {!c && <span>Klinik Kılavuzu</span>}
              </Link>
            </NavTooltip>
          </div>
        </div>
        ...
        {(isAdmin || canSeeFinance) && (
          <div>
            ...
            <div className="space-y-1 mt-1">
              {isAdmin && (
                <NavTooltip label="Raporlar" show={c}>
                  <Link id="tour-reports" href={`${base}/reports`} className={linkClass(`${base}/reports`)} style={activeStyle(`${base}/reports`)} onClick={handleClick}>
                    {indicator(`${base}/reports`)}
                    <svg className={iconCls(`${base}/reports`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                    {!c && <span>Raporlar</span>}
                  </Link>
                </NavTooltip>
              )}
              {canSeeFinance && (
                <NavTooltip label="Karlılık & Hak Ediş" show={c}>
                  <Link href={`${base}/finance`} className={linkClass(`${base}/finance`)} style={activeStyle(`${base}/finance`)} onClick={handleClick}>
                    {indicator(`${base}/finance`)}
                    <svg className={iconCls(`${base}/finance`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                    {!c && <span>Karlılık & Hak Ediş</span>}
                  </Link>
                </NavTooltip>
              )}
            </div>
        ...
        <div>
          ...
          <div className="space-y-1 mt-1">
            <NavTooltip label="Ekip ve Yetkiler" show={c}>
              <Link href={`${base}/admin/users`} className={linkClass(`${base}/admin/users`)} style={activeStyle(`${base}/admin/users`)} onClick={handleClick}>
                {indicator(`${base}/admin/users`)}
                <svg className={iconCls(`${base}/admin/users`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 0 0 3.741-.479 3 3 0 0 0-4.682-2.72m.94 3.198.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0 1 12 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 0 1 6 18.719m12 0a5.971 5.971 0 0 0-.941-3.197m0 0A5.995 5.995 0 0 0 12 12.75a5.995 5.995 0 0 0-5.058 2.772m0 0a3 3 0 0 0-4.681 2.72 8.986 8.986 0 0 0 3.74.477m.94-3.197a5.971 5.971 0 0 0-.94 3.197M15 6.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Zm6 3a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Zm-13.5 0a2.25 2.25 0 1 1-4.5 0 2.25 2.25 0 0 1 4.5 0Z" /></svg>
                {!c && <span>Ekip ve Yetkiler</span>}
              </Link>
            </NavTooltip>
            <NavTooltip label="Abonelik & Destek" show={c}>
              <Link id="tour-subscription" href={`${base}/admin/subscription`} className={linkClass(`${base}/admin/subscription`)} style={activeStyle(`${base}/admin/subscription`)} onClick={handleClick}>
                {indicator(`${base}/admin/subscription`)}
                <svg className={iconCls(`${base}/admin/subscription`)} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 0 0 2.25-2.25V6.75A2.25 2.25 0 0 0 19.5 4.5h-15a2.25 2.25 0 0 0-2.25 2.25v10.5A2.25 2.25 0 0 0 4.5 19.5Z" /></svg>
                {!c && <span>Abonelik & Destek</span>}
              </Link>
            </NavTooltip>
          </div>
        </div>
...
function ShellInner({ children }: Props) {
  ...
  const linkClass = useCallback((href: string, forMobile = false) => {
    const effectiveCollapsed = sidebarCollapsed && !forMobile;
    return [
      "group relative flex items-center text-[13px] font-bold rounded-2xl transition-all duration-300 overflow-hidden",
      effectiveCollapsed ? "justify-center py-2.5 w-full" : "gap-2.5 px-4 py-2.5",
      isActive(href)
        ? (isPlatform && clinic.isSuperAdmin)
          ? "bg-slate-900 text-white shadow-xl shadow-slate-900/10"
          : effectiveCollapsed ? "text-white scale-[1.01]" : "text-white scale-[1.02] translate-x-1"
        : effectiveCollapsed
          ? "text-slate-500 hover:text-slate-900 hover:bg-slate-50/50"
          : "text-slate-500 hover:text-slate-900 hover:bg-slate-50/50 hover:translate-x-1",
    ].join(" ");
  }, [isActive, isPlatform, clinic.isSuperAdmin, sidebarCollapsed]);
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex w-full transition-all duration-500 ease-in-out">
        <aside className={`hidden md:flex ${sidebarCollapsed ? "w-20" : "w-64"} flex-col border-r border-r-slate-100 bg-white h-screen sticky top-0 z-50 transition-all duration-300 relative group/sidebar ${isFullOverlay ? "scale-[0.99] grayscale-[0.05] pointer-events-none" : ""}`}>
          {/* Toggle Button */}
          <button
            onClick={toggleSidebar}
            className="absolute -right-4 top-24 z-[60] h-8 w-8 rounded-full border-2 border-white flex items-center justify-center text-white shadow-xl transition-all duration-300 group-hover/sidebar:opacity-100 opacity-60 hover:opacity-100 hover:scale-110 active:scale-95"
            style={{ background: `linear-gradient(135deg, var(--brand-from), var(--brand-to))` }}
          >
            <svg className={`h-4 w-4 transition-transform duration-500 ${sidebarCollapsed ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor">
          ...
          <nav className={`flex-1 ${sidebarCollapsed ? "px-3" : "px-4"} text-sm flex flex-col min-h-0 overflow-y-auto overflow-x-visible custom-scrollbar pt-2`}>
            <div className="space-y-4 flex-1 pb-8">
              {(clinic.isSuperAdmin && isPlatform) ? (
                <div className="space-y-1">
                  <SuperAdminNav linkClass={linkClass} collapsed={sidebarCollapsed} />
                </div>
              ) : (
                <ClinicNav linkClass={linkClass} slug={clinic.clinicSlug || ""} isAdmin={clinic.isAdmin || clinic.isSuperAdmin} userRole={clinic.userRole ?? undefined} collapsed={sidebarCollapsed} />
              )}
            </div>
            <div className={`mt-auto pt-4 pb-2 flex-shrink-0 ${sidebarCollapsed ? "space-y-2 px-1" : "space-y-1.5"}`}>
              <div className={sidebarCollapsed ? "flex flex-col gap-2" : "flex gap-1.5"}>
                <NavTooltip label="Tema Ayarları" show={!!sidebarCollapsed}>
                  <button
                    type="button"
                    onClick={() => setThemeSettingsOpen(true)}
                    className={`flex items-center justify-center rounded-xl border border-slate-100 bg-slate-50/50 text-slate-500 hover:bg-white hover:text-slate-900 h-10 text-[11px] font-black transition-all shadow-sm ${sidebarCollapsed ? "w-10" : "flex-1 gap-2 mx-auto"}`}
                  >
                ...
                {canSwitchClinics && (
                  <NavTooltip label="Şube Değiştir" show={!!sidebarCollapsed}>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      {!sidebarCollapsed && <span>Şube</span>}
                    </button>
                  </NavTooltip>
                )}
              </div>

              <NavTooltip label="Çıkış Yap" show={!!sidebarCollapsed}>
                <button
                  type="button"
                  onClick={handleLogout}
                  className={`flex items-center justify-center rounded-xl bg-rose-50/50 text-rose-500 hover:bg-rose-50 hover:text-rose-600 h-10 text-[11px] font-black transition-all ${sidebarCollapsed ? "w-10" : "w-full gap-2"}`}
                >
                  ...
                  <div className="bg-white/95 backdrop-blur-2xl rounded-[2.5rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.2)] overflow-hidden ring-1 ring-white/20 border border-slate-100">
                    <div className="p-6">
                      ...
                      <nav className="space-y-1 max-h-[55vh] overflow-y-auto px-1 pr-2 custom-scrollbar">
                        {(clinic.isSuperAdmin && isPlatform) ? (
                          <SuperAdminNav linkClass={(href) => linkClass(href, true)} onNav={closeMobileNav} collapsed={false} />
                        ) : (
                          <ClinicNav
                            linkClass={(href) => linkClass(href, true)}
